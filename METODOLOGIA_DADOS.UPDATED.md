...
- **Alerta (multi-risco · fase de teste)** = **pior leitura entre as estações online**,
  em quatro perigos, com três níveis (Normal/Atenção/Alerta):
  - **Chuva** — classes de intensidade da **OMM** (mm/h): leve < 2,5; moderada 2,5–10;
    forte 10–50; violenta ≥ 50; alerta a partir de **forte** (≥ 10). Convenção operacional.
  - **Vento** — escala de **Beaufort** (OMM, *Manual on Codes*, WMO-No. 306), vento
    sustentado a 10 m: força 6 "vento forte" ≥ 10,8 m/s (Atenção); força 8 "ventania"
    ≥ 17,2 m/s (Alerta). Rajadas são maiores que o vento sustentado.
  - **Temperatura** (ar) — bandas operacionais **provisórias**: calor ≥ 32/≥ 36 °C;
    frio–geada ≤ 5/≤ 3 °C (a geada forma-se ~0 °C na superfície; a banda de ar a 2 m
    antecipa o risco). Pendentes de critério local da Defesa Civil.
  - **Sensação (índice de calor)** — **temperatura aparente** (temperatura × umidade), o
    risco real numa cidade úmida. Base científica: **Steadman (1979)**, *J. Appl. Meteorol.*
    18(7):861–873; fórmula operacional do **NWS** (**Rothfusz, 1990**, SR/SSD 90-23);
    Atenção ≥ 32, Alerta ≥ 41 °C aparente. Calculada por estação (temp + UR frescas).
  Todos são **orientação, não aviso oficial** da Defesa Civil.
...
- Baixa os GRIB2 do CPTEC para a caixa de Joinville e extrai chuva (`tp/acpcp/ncpcp`),
  temperatura 2 m (`t2m`/`2t`/`t@2m`), vento 10 m (`u10`/`v10`) e — quando o produto a
  disponibiliza — **umidade relativa a 2 m** (`r2`/`2r`/`r@2m` → `rh2_pct`), usada no
  alerta de sensação. A ausência de UR é tolerada (o campo é simplesmente omitido).
- **Acumulado × instantâneo (a regra central, verificada nas notas do laboratório):**
  ...
  - Salva `wrf_joinville_<run>Z.nc` (`precip_mm_h`, `t2m_degC`, `wspd10_ms`, `wdir10_deg`,
    `u10_ms`, `v10_ms`, `rh2_pct` quando presente).
...
- `wrf_basins.csv`, `wrf_bairros.csv` — resumos por bacia e por bairro (download);
  ...
  **local** (janela de Joinville) + **regional** (caixa toda, para o mapa de retalhos) +
  séries por bacia/bairro (chuva, temperatura, vento e — quando há UR a 2 m — **umidade**,
  com `has.humid` sinalizando a presença). Mapas sobrepõem `limite.geojson` + `bacias.geojson` + `bairros.geojson`.
...
### 7.6 Escopo
- Saída **direta** do modelo (sem correção de viés nem calibração local). Cobertura depende
  da retenção do CPTEC. **Atualização automática já ativa**: `forecast-wrf.yml` roda
  `fetch_wrf.py` + `build_wrf_basins.py` diariamente e publica a rodada mais recente
  (mesmo padrão de `fetch_airport.py` / `build_enso.py`).

### 7.7 Alertas de risco (previsão) — `renderForecastAlert` em `previsao.html`
Espelha o alerta observado da página **Agora**, mas sobre a **previsão**: toma o **pior
valor entre os bairros** no instante escolhido (ou na janela futura), em quatro perigos e
três níveis, com limiares **idênticos** aos da Agora:
- **Chuva** (OMM): moderada ≥ 2,5 · forte ≥ 10 · violenta ≥ 50 mm h⁻¹ — alerta em forte.
- **Vento** (Beaufort, WMO-No. 306; sustentado a 10 m): Atenção ≥ 10,8 · Alerta ≥ 17,2 m/s.
- **Temperatura** (ar 2 m, provisório): calor ≥ 32/≥ 36 °C · frio–geada ≤ 5/≤ 3 °C.
- **Sensação** (índice de calor; Steadman 1979 / Rothfusz 1990): Atenção ≥ 32 · Alerta ≥ 41 °C
  aparente. **Exige** a umidade a 2 m prevista (`rh2_pct` → `vars.humid` / `has.humid`); se o
  WRF não trouxer UR, o item não aparece. **Fase de teste**, não é aviso oficial.

### 7.8 "Acumulado" e séries horárias — janela das próximas 24 h
- O modo **"Acumulado"** da chuva soma as **próximas 24 h a partir do instante atual**
  (mesma janela do gráfico "Evolução horária por bacia"), **não** a rodada inteira —
  coerente entre barras, bairros e mapa. O eixo dos gráficos horários é rotulado em
  **hora local** (HH:MM), com o lead (+X h) como legenda secundária.

### 7.9 Arquivo histórico da grade — chunk anual + DOI
- A cada rodada, `archive_wrf_grid.py` **acrescenta** os valores por célula (janela de
  Joinville, hora a hora) a arquivos **fatiados por ano** em `site/data/wrf_archive/`
  (`grade_AAAA.csv` + `.csv.gz` + `index.json`), apenas-acréscimo e idempotente; cada ano
  fechado é **imutável** (evita o limite de 100 MB/arquivo do GitHub e o inchaço do histórico
  git). Download por **ano/período** na página. Preservação de longo prazo e **DOI** citável
  via **Zenodo** por *release* — ver `DATA_ARCHIVE.md`.
...
- Alertas de risco: **chuva** = classes de intensidade da OMM; **vento** = escala de Beaufort (OMM, *Manual on Codes*, WMO-No. 306); **temperatura** = bandas operacionais provisórias (pendentes de critério local); **sensação / índice de calor** = Steadman, R. G. (1979), *The Assessment of Sultriness, Part I*, J. Appl. Meteorol. 18(7):861–873 (DOI 10.1175/1520-0450(1979)018<0861:TAOSPI>2.0.CO;2), implementado pela regressão do NWS (Rothfusz, 1990, SR/SSD 90-23). Direção de vento resultante: Grange, S. K. (2014).
